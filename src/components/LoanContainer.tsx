...
function LoanContainer({ loans, setLoans, openModal, recordType }: any) {
  ...
  const [showMarkAsPaidAlert, setShowMarkAsPaidAlert] =
    useState<boolean>(false); // Estado para la alerta de marcar como pagado
  ...
  const [slidingLoanToMarkAsPaid, setSlidingLoanToMarkAsPaid] =
    useState<any>(null); // Estado para el sliding del préstamo a marcar como pagado
  ...
  const filterAndSortLoans = () => {
    ...
    if (sortCriteria === "paid") {
      ...
      result = result.filter(
        (loan: Loan) => !loan.isPayed && loan.payDate < currentDate
      );
  ...
  const handleDeleteLoan = async (loan: Loan) => {
    ...
    if (loanElement) {
      ...
      setTimeout(async () => {
        ...
        setLoans((prevState: Loan[]) => prevState.filter((l: Loan) => l.id !== loan.id));
      }, 300);
    }
  };
  ...
  const handleMarkAsPaid = async (loan: Loan, slidingItem: any) => {
    ...
    setLoans((prevState: Loan[]) => prevState.map((l: Loan) => (l.id === loan.id ? updatedLoan : l)));
  ...
  const getColor = (loan: Loan) => {
    ...
    const daysUntilPay = Math.ceil(
      (new Date(loan.payDate).getTime() - currentDate) / (1000 * 60 * 60 * 24)
    );
  ...
  return (
    <>
      <IonHeader>
        <IonToolbar>
          <IonTitle>
            {recordType === "loan" ? "Tus Préstamos" : "Tus Deudas"}
          </IonTitle>
        </IonToolbar>
      </IonHeader>
      <IonContent>
              ...
              <IonRow>
                ...
                <IonCol pull="1" className="col-filter">
                  ...
                  >
                    ...
                    <IonSelectOption value="duedateasc">
                      Fecha (Ascendente)
                    </IonSelectOption>
                    <IonSelectOption value="duedatedesc">
                      Fecha (Descendente)
                    </IonSelectOption>
                  </IonSelect>
                </IonCol>
                  ...
                  <IonItemOptions side="end">
                    <IonItemOption
                      ...
                      onClick={(e: any) =>
                        confirmMarkAsPaid(
                          loan,
                          e.target.closest("ion-item-sliding")
                        )
                      }
                    ...
                    <IonItemOption
                      ...
                      color="danger"
                      onClick={(
                        e: React.MouseEvent<HTMLIonItemOptionElement>
                      ) =>
                        confirmDeleteLoan(
                          loan,
        ...
        <IonAlert
          ...
          message={`¿Está seguros de que deseas eliminar el préstamo a "${loanToDelete?.name}"? Esta acción no se puede deshacer.`}
          buttons={[
            {
              text: "Cancelar",
              role: "cancel",
        ...
        <IonAlert
          ...
          message={`¿Estás seguro de que deseas marcar el préstamo a ${loanToMarkAsPaid?.name} como pagado?`}
          buttons={[
            {
              text: "Cancelar",
              role: "cancel",
